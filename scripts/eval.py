#!/usr/bin/env python
"""
using the filtered correlation helpers from scripts.metrics:
  only voxels with soft_clip < soft < 1-soft_clip are used.
"""
...
CONFIG = {
    ...
    "soft_clip": 0.05,             # remove soft≈0 and soft≈1
}
...
def collate_fn_factory(target_shape: Tuple[int, int, int]):
    def _collate(batch):
        ...
        tgt = b["target"].unsqueeze(0).unsqueeze(0)        # [1,1,D,H,W], soft label
        dis = b["disagreement"].unsqueeze(0).unsqueeze(0)  # [1,1,D,H,W] (unused here)
...
def main():
    ...
    # for GLOBAL correlation, we’ll collect filtered voxels
    all_soft = []
    all_alea = []
    all_epi = []
    ...
    for img, tgt, dis, meta in val_loader:
        img, tgt = img.to(device), tgt.to(device)  # tgt: [1,1,D,H,W]

        prob_stack = ensemble_forward(models, img)        # [K,1,D,H,W]
        p_mean = prob_stack.mean(dim=0)                   # [1,1,D,H,W]
        alea_hat = bernoulli_entropy(prob_stack).mean(dim=0)  # [1,1,D,H,W]
        p_entropy = bernoulli_entropy(p_mean)                 # [1,1,D,H,W]
        epi_hat = p_entropy - alea_hat                        # [1,1,D,H,W]

        # --- per-case correlation using metrics helpers (with filtering) ---
        c_alea = corr_alea_softlabel(alea_hat, tgt, soft_clip=soft_clip)
        c_epi = corr_epi_softlabel(epi_hat, tgt, soft_clip=soft_clip)
        per_case_corr_alea.append(c_alea)
        per_case_corr_epi.append(c_epi)
        case_ids.append((meta["patient"], meta["nodule"]))

        print(f"{meta['patient']}/{meta['nodule']}: "
              f"corr(alea,soft)={c_alea:.4f}, corr(epi,soft)={c_epi:.4f}")

        # --- collect filtered voxels for global correlation ---
        soft_np = tgt.squeeze().cpu().numpy().ravel()
        alea_np = alea_hat.squeeze().cpu().numpy().ravel()
        epi_np = epi_hat.squeeze().cpu().numpy().ravel()

        mask = (soft_np > soft_clip) & (soft_np < 1.0 - soft_clip)
        soft_np = soft_np[mask]
        alea_np = alea_np[mask]
        epi_np = epi_np[mask]
    ...
    # Global correlations (also on filtered voxels)
    if len(all_soft) > 0:
    ...
    print("\nGlobal correlations (filtered soft labels):")
