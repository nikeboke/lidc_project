...
Compute disentanglement metrics for AU/EU/Soft on LIDC:
...
def collate_fn(batch):
    ...
    Dataset item is a dict with:
      ...
      - "disagreement": [D,H,W] (not used in this script)
      - "meta": {"patient": ..., "nodule": ...}
...
def compute_slice_corrs(alea_slice, epi_slice, soft_slice):
    """
    alea_slice, epi_slice, soft_slice: np.ndarray [H,W] for a single slice.
    """
...
def main():
    ...
    models = load_models(args.ckpt_dir, args.base, device)
    loader = build_val_loader(args.data_root)

    rows_all = []
    patient_level_stats = []  # for printing and later summary
    ...
    for img, tgt, dis, meta in loader:
        ...
        alea = bernoulli_entropy(ps).mean(0)     # [1,1,D,H,W] -> E[H[p]]
        ...
        for d in range(D):
            ...
            res = compute_slice_corrs(a_slice, e_slice, s_slice)
        ...
        patient_level_stats.append((patient, nodule, mean_alea, mean_epi))
    ...
    # ----- Save all-slice CSV -----
    out_all = os.path.join(args.ckpt_dir, "uncertainty_correlations_all.csv")
    ...
    out_valid = os.path.join(args.ckpt_dir, "uncertainty_correlations_valid.csv")
    ...
    out_nan = os.path.join(args.ckpt_dir, "uncertainty_correlations_nan.csv")
    ...
    out_patient = os.path.join(args.ckpt_dir, "uncertainty_correlations_patient.csv")
